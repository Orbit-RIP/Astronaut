 ...
 */
public class VanishSlotMenu extends Menu {
	@Override
		...
		for (int i = 0; i < 9; i++) {
			...
			buttons.add(new Button() {
				...
				public ItemStack stack(Player player) {
					if (loadOut.getVanishSlot() == (finalI)) {
				...
				public void action(Player player, int slot, InventoryClickEvent event) {
					loadOut.setVanishSlot((finalI));
					loadOut.save();
					player.sendMessage(CC.chat("&cSet your vanish slot to the #" + (finalI + 1) + " slot."));
