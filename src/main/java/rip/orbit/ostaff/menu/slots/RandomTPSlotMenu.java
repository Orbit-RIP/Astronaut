 ...
 */
public class RandomTPSlotMenu extends Menu {
	@Override
		...
		for (int i = 0; i < 9; i++) {
			...
			buttons.add(new Button() {
				...
				public ItemStack stack(Player player) {
					if (loadOut.getRandomTPSlot() == (finalI)) {
				...
				public void action(Player player, int slot, InventoryClickEvent event) {
					loadOut.setRandomTPSlot((finalI));
					loadOut.save();
					player.sendMessage(CC.chat("&cSet your random tp slot to the #" + (finalI + 1) + " slot."));
