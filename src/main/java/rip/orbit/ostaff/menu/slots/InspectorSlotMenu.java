 ...
 */
public class InspectorSlotMenu extends Menu {
	@Override
		...
		for (int i = 0; i < 9; i++) {
			...
			buttons.add(new Button() {
				...
				public ItemStack stack(Player player) {
					if (loadOut.getInspectorSlot() == (finalI)) {
				...
				public void action(Player player, int slot, InventoryClickEvent event) {
					loadOut.setInspectorSlot((finalI));
					loadOut.save();
					player.sendMessage(CC.chat("&cSet your inspector slot to the #" + (finalI + 1) + " slot."));
