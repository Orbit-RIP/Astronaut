 ...
 */
public class WorldEditSlotMenu extends Menu {
	@Override
		...
		for (int i = 0; i < 9; i++) {
			...
			buttons.add(new Button() {
				...
				public ItemStack stack(Player player) {
					if (loadOut.getWorldEditSlot() == (finalI)) {
				...
				public void action(Player player, int slot, InventoryClickEvent event) {
					loadOut.setWorldEditSlot((finalI));
					loadOut.save();
					player.sendMessage(CC.chat("&cSet your worldedit slot to the #" + (finalI + 1) + " slot."));
