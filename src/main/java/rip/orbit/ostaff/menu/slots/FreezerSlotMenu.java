 ...
 */
public class FreezerSlotMenu extends Menu {
	@Override
		...
		for (int i = 0; i < 9; i++) {
			...
			buttons.add(new Button() {
				...
				public ItemStack stack(Player player) {
					if (loadOut.getFreezerSlot() == (finalI)) {
				...
				public void action(Player player, int slot, InventoryClickEvent event) {
					loadOut.setFreezerSlot((finalI));
					loadOut.save();
					player.sendMessage(CC.chat("&cSet your freezer slot to the #" + (finalI + 1) + " slot."));
