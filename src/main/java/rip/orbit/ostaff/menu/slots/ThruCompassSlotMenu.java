 ...
 */
public class ThruCompassSlotMenu extends Menu {
	@Override
		...
		for (int i = 0; i < 9; i++) {
			...
			buttons.add(new Button() {
				...
				public ItemStack stack(Player player) {
					if (loadOut.getCompassSlot() == (finalI)) {
				...
				public void action(Player player, int slot, InventoryClickEvent event) {
					loadOut.setCompassSlot((finalI));
					loadOut.save();
					player.sendMessage(CC.chat("&cSet your thru compass slot to the #" + (finalI + 1) + " slot."));
